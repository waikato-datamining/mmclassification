...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G \
    ...
    -it public.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmclassification:0.25.0_cuda11.1
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --gpus=all --shm-size 8G \
    -v /local/dir:/container/dir \
    -it waikatodatamining/mmclassification:0.25.0_cuda11.1
  ```

### Build local image

* Build the image from Docker file (from within /path_to/mmclassification/0.25.0_cuda11.1)
...
```bash
docker build -t mmclassification:0.25.0_cuda11.1 .
```
...
* Tag
  ...
  ```bash
  docker tag \
    mmclassification:0.25.0_cuda11.1 \
    public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmclassification:0.25.0_cuda11.1
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/open-mmlab/mmclassification:0.25.0_cuda11.1
  ```
...
* Push
  ...
  ```bash
  docker push waikatodatamining/mmclassification:0.25.0_cuda11.1
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
